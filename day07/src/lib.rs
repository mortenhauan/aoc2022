...
fn recursive(lines: &Vec<&str>, position: usize, total: &mut usize) -> (usize, usize) {
    ...
    while position < lines.len() {
        ...
        position += 1;
        match line {
            "$ cd .." => return (sum, position),
            _ if line.starts_with("$ cd") => {
                let result;
                (result, position) = recursive(lines, position, total);
                sum += result;
                if result <= 100000 {
                    *total += result;
                }
            }
            _ if line.starts_with("$ ls") => continue,
            _ if line.starts_with("dir") => continue,
            _ => {
                let size = line
                    .split_whitespace()
                    .next()
...
fn find_big_enough(
    ...
    while position < lines.len() {
        ...
        match line {
            ...
            _ if line.starts_with("$ cd") => {
                let result;
                (result, position) = find_big_enough(lines, position, folders, space);
                sum += result;
                if result >= space {
                    folders.push(result);
                }
...
fn total_size(commands: &str) -> usize {
    let mut total = 0;
...
/// (`94853` + `584`). (As in this example, this process can count files more
/// than once!)
///
/// # Example
...
pub fn part1(input: &str) -> usize {
    ...
    let mut total = 0;
    let (_, _) = recursive(&lines, 0, &mut total);
    total
}
...
pub fn part2(input: &str) -> usize {
    let needed_space = total_size(input) - 40_000_000;

    let mut big_enough_folders = Vec::<usize>::new();

    let lines: Vec<&str> = input.lines().collect();
    let (_, _) = find_big_enough(&lines, 0, &mut big_enough_folders, needed_space);

    *big_enough_folders.iter().min().unwrap()
}
...
7214296 k";
        ...
        assert_eq!(total_size(input), 48381165);
    }
}
